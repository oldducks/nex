...
const START_IDS = new Set(["profile", "catalog", "landing-pages"]);
const TOOL_IDS = new Set(["qr", "design", "leads", "referrals", "learning", "analytics", "ai"]);
const ACCOUNT_IDS = new Set(["upgrade-plan", "account"]);
...
export default function ControlCenterV2Page() {
  ...
  const { startItems, toolItems, accountItems } = useMemo(() => {
    return {
      startItems: MENU_ITEMS.filter((item) => START_IDS.has(item.id)),
      toolItems: MENU_ITEMS.filter((item) => TOOL_IDS.has(item.id)),
      accountItems: MENU_ITEMS.filter((item) => ACCOUNT_IDS.has(item.id)),
    };
  }, []);
  ...
  return (
    <main className="min-h-screen bg-[#EEF0FF] text-[#0F172A]">
      <section className="mx-auto flex min-h-screen w-full max-w-md flex-col px-4 pb-8 pt-5">
        ...
        <GridSection title="เริ่มต้น" items={startItems} withPrefix={withPrefix} isFeatureEnabled={isFeatureEnabled} />
        <GridSection title="เครื่องมือ" items={toolItems} withPrefix={withPrefix} isFeatureEnabled={isFeatureEnabled} />
        <GridSection title="บัญชี" items={accountItems} withPrefix={withPrefix} isFeatureEnabled={isFeatureEnabled} />
...
function GridSection({
  ...
  return (
    <section className="mb-4 rounded-[28px] border border-[#D9E1F2] bg-white p-5 shadow-[0_24px_60px_-40px_rgba(5,5,121,0.2)]">
      <h2 className="mb-3 text-sm font-bold text-[#050579]">{title}</h2>
