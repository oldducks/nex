...
const MENU_ITEMS: MenuItem[] = [
  { id: "profile", label: "นามบัตรดิจิทัล", href: "/manage/profile", icon: Smartphone },
  { id: "catalog", label: "อี แคตตาล็อค", href: "/manage", icon: BookOpen },
  { id: "landing-pages", label: "หน้าร้านออนไลน์", href: "/manage/landing-pages", icon: Layout },
  { id: "qr", label: "สร้าง QR Code", href: "/manage/qr", icon: QrCode },
  { id: "forms", label: "ฟอร์มบันทึกข้อมูล", href: "/manage/forms", icon: FileText },
  { id: "referrals", label: "พันธมิตรธุรกิจ", href: "/manage/referrals", icon: Gift },
  { id: "analytics", label: "ข้อมูลสรุป", href: "/manage/dashboard", icon: BarChart3 },
  { id: "ai", label: "สร้างงานด้วย AI", href: "/manage/digital-media-v1", icon: Zap },
  { id: "learning", label: "คู่มือการใช้งาน", href: "/manage/learning", icon: UserCircle },
  { id: "design", label: "ออกแบบกราฟิก", href: "/manage/create-lite", icon: ImageIcon },
  ...
  useEffect(() => {
    ...
    const fetchMe = async () => {
      try {
        ...
        await (res.json() as Promise<MeResponse>);
      } catch {
      ...
      <section className="mx-auto w-full max-w-md px-4 pb-8 pt-5">
        <header className="rounded-3xl border border-[#D9E1F2] bg-white p-5 shadow-[0_18px_40px_-30px_rgba(5,5,121,0.35)]">
          ...
          <p className="mt-2 text-sm leading-relaxed text-[#475569]">
            สถานะตอนนี้ : สมาชิกทั่วไป (ฟังก์ชันการใช้งานบางส่วนถูกจำกัด)
          </p>
        </header>
        ...
        <MenuGroup title="เครื่องมือหลัก" items={coreItems} withPrefix={withPrefix} />
        <MenuGroup title="การเติบโตและวิเคราะห์" items={growthItems} withPrefix={withPrefix} />
        <MenuGroup title="การตั้งค่าและเมนูเพิ่มเติม" items={settingItems} withPrefix={withPrefix} />
