...
export default function CatalogManageV3Page() {
  ...
  return (
    <div className="relative min-h-screen bg-[#EEF0FF] pb-20 text-[#0F172A]">
        ...
        <div className="relative mx-auto flex h-24 w-full max-w-md items-center px-4 md:max-w-5xl md:px-6">
          ...
          <div className="mx-auto flex min-w-0 flex-col items-center text-center">
            ...
            <div className="mt-2 inline-flex items-center rounded-full border border-[#D9E1F2] bg-[#F6F8FF] px-3 py-1 text-xs font-bold text-[#64748B]">
              <span className="mr-1.5 text-[#94A3B8]">รายการทั้งหมด</span>
              <span className="text-[#050579]">{catalogs.length}</span>
            </div>
          </div>
                  ...
                  >
                    <div className="px-4 py-4">
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              ...
                              <p className="mt-2 line-clamp-2 text-sm leading-6 text-[#64748B]">
                                {catalog.description || "ยังไม่ได้เพิ่มคำอธิบายสำหรับแคตตาล็อกนี้"}
                              </p>
                            </div>
                            <span
                              className={`shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase ${
                                catalog.pdf_url
                                  ? "border-[#D9EFD2] bg-white text-[#16A34A]"
                                  : "border-[#D9E1F2] bg-white text-[#64748B]"
                              }`}
                            >
                              {catalog.pdf_url ? "PDF Ready" : "Draft"}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 border-t border-[#DEE7F7] pt-4">
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_56px]">
                          <button
                            onClick={() => openEditModal(catalog)}
                            className="flex items-center justify-between rounded-2xl border border-[#D1DBEF] bg-[#EEF4FF] px-3 py-3 text-sm font-semibold text-[#64748B] transition hover:border-[#BCCBE8] hover:bg-white hover:text-[#050579]"
                          ...
                          <button
                            ...
                            className="flex items-center justify-between rounded-2xl border border-[#D1DBEF] bg-[#EEF4FF] px-3 py-3 text-sm font-semibold text-[#64748B] transition hover:border-[#BCCBE8] hover:bg-white hover:text-[#050579] disabled:opacity-60"
                            title="สร้าง PDF ใหม่"
                          >
                            <span className="flex items-center gap-2">
                              {generatingId === catalog.id ? (
                                <Loader2 size={15} className="animate-spin" />
                              ) : (
                                <Settings size={15} />
                              )}
                              {generatingId === catalog.id ? "กำลังสร้าง" : "PDF"}
                            </span>
                          </button>
                          <button
                    ...
                    <div className="border-t border-[#DEE7F7] bg-[#EDF4FF] px-4 py-4">
                      <div className="flex items-center justify-between gap-4 rounded-2xl border border-[#D1DBEF] bg-[#F7FAFF] px-4 py-3">
                        <div>
                          <div className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#94A3B8]">Catalog Preview</div>
                          <div className="mt-1 text-sm font-semibold text-[#050579]">QR สำหรับแชร์และตัวอย่างสินค้า 4 รูป</div>
                        </div>
                        <div className="rounded-2xl border border-[#E2E8F0] bg-white p-2 shadow-sm">
                          <QrCodeImage url={getCatalogUrl(catalog.id)} size={64} />
                        </div>
                      </div>
                        ...
                        {Array.from({ length: 4 }).map((_, index) => {
                          ...
                          if (!preview) {
                            return (
                              <div
                                ...
                                className="flex aspect-[4/3] items-center justify-center rounded-2xl border border-dashed border-[#D1DBEF] bg-white/92"
                              >
                          ...
                          return (
                            <div
                              ...
                              className="aspect-[4/3] overflow-hidden rounded-2xl border border-[#D1DBEF] bg-white"
                            >
      ...
      {shareModalCatalog && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#050579]/40 backdrop-blur-md" onClick={() => setShareModalCatalog(null)} />
          <div className="relative z-10 w-full max-w-md rounded-[32px] border border-[#D9E1F2] bg-white p-8 shadow-2xl">
            <button
              onClick={() => setShareModalCatalog(null)}
              className="absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-full bg-[#F6F8FF] text-[#64748B] transition-colors hover:bg-[#EEF0FF]"
            >
              <X size={20} />
            </button>

            <h2 className="mb-2 text-2xl font-black text-[#050579]">แชร์แคตตาล็อก</h2>
            <p className="mb-6 text-sm font-medium text-[#64748B]">{shareModalCatalog.title}</p>

            <div className="mb-6 rounded-2xl border border-[#D1DBEF] bg-[#F7FAFF] p-4">
              <div className="mx-auto w-full max-w-[260px] rounded-[28px] border border-[#D9E1F2] bg-white p-4 shadow-sm">
                <div className="mb-3 text-center text-[10px] font-bold uppercase tracking-[0.16em] text-[#94A3B8]">QR Code</div>
                <QrCodeImage url={getCatalogUrl(shareModalCatalog.id)} size={170} />
                <div className="mt-3 text-center text-sm font-semibold text-[#050579]">{shareModalCatalog.title}</div>
                <div className="mt-1 break-all text-center text-[11px] leading-5 text-[#64748B]">{getCatalogUrl(shareModalCatalog.id)}</div>
                <QrCodeDownloadActions
                  qrValue={getCatalogUrl(shareModalCatalog.id)}
                  fileBaseName={`catalog-${shareModalCatalog.id}`}
                  titleLine="QR Code"
                  nameLine={shareModalCatalog.title}
                  bottomLabel="URL"
                  bottomLine={getCatalogUrl(shareModalCatalog.id)}
                  className="mt-4"
                />
              </div>
            </div>

            <div className="mb-6 rounded-2xl border border-[#D1DBEF] bg-[#F7FAFF] px-4 py-3">
              ...
              <p className="break-all text-sm font-semibold text-[#050579]">{getCatalogUrl(shareModalCatalog.id)}</p>
            </div>

            <div className="grid grid-cols-4 gap-3">
              <button
                onClick={() => shareToFacebook(shareModalCatalog.id)}
                className="flex flex-col items-center gap-2 rounded-xl bg-[#1877F2]/10 p-4 text-[#1877F2] transition-all hover:bg-[#1877F2] hover:text-white"
              >
                <FacebookIcon />
              ...
              <button
                ...
                className="flex flex-col items-center gap-2 rounded-xl bg-[#00B900]/10 p-4 text-[#00B900] transition-all hover:bg-[#00B900] hover:text-white"
              >
                <LineIcon />
              ...
              <button
                ...
                className="flex flex-col items-center gap-2 rounded-xl bg-[#25D366]/10 p-4 text-[#25D366] transition-all hover:bg-[#25D366] hover:text-white"
              >
                <WhatsAppIcon />
              ...
              <button
                ...
                className={`flex flex-col items-center gap-2 rounded-xl p-4 transition-all ${
                  copiedId === shareModalCatalog.id ? "bg-[#16A34A] text-white" : "bg-[#E5E7EB] text-[#64748B] hover:bg-[#050579] hover:text-white"
                }`}
              >
