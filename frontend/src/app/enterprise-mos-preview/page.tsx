...
const storyboardImages = Array.from({ length: 17 }, (_, index) => {
  ...
  return {
    order: frameNumber,
    src: `/enterprise-mos-preview/${frameNumber}.jpg`,
    alt: `Enterprise MOS preview frame ${frameNumber}`,
  };
});

const REGISTER_BUTTON_AFTER = new Set([1, 7, 13, 15, 17]);

export default function EnterpriseMosPreviewPage() {
  return (
    <main className="bg-[#EEF0FF]">
      <MarketingPageTracker pageKey="enterprise-mos-preview" />
              ...
              {image.order === 1 && (
                <section className="relative overflow-hidden pt-2 lg:flex lg:min-h-dvh lg:items-center lg:justify-center">
                  <div className="w-full overflow-hidden bg-black lg:w-auto">
                    <PreviewVideo
                      src="/enterprise-mos-preview/preview-video.mp4"
                      pageKey="enterprise-mos-preview"
